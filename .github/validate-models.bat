...
REM
REM The list is deliberately UNQUOTED (no filename contains spaces): the previous
REM form set "MODELS=..." "..." embedded literal quotes into the variable, which
REM made the for-loop see the whole list as ONE token — `if not exist` then parsed
REM it as path-plus-command, so only a fragment was ever checked and the
REM exit /b 1 never fired (the ERROR printed but the step passed; observed in run
REM 28805360584, where an empty model cache sailed through this "gate").
