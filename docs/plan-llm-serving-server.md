...
- **M2 — Utilization/observability.** Extend `ServerMetrics` (GPU/VRAM, queue, prompt-cache, per-model) and surface on `/metrics` + Java API.
- **M3 — Multi-model admin.** `ModelPool` facade: list/load/unload + per-model health/metrics in router mode.
- **M4 — KV-cache admin.** `POST/DELETE /slots`, prompt-cache stats readout.
- **M5 — Multi-client.** API-key plumbing on `OpenAiServerConfig`, optional token-bucket rate limiter, concurrency defaults.
