...
- **M4 — KV-cache admin.** `POST/DELETE /slots`, prompt-cache stats readout. **Status: slot save/erase now functional.** `POST /slots` (save) and `DELETE /slots/:id` (erase) are registered on `OpenAiCompatServer` and now delegate to `LlamaModel.saveSlot` / `eraseSlot`, which call the pre-existing native `handleSlotAction` (`SERVER_TASK_TYPE_SLOT_SAVE` / `SLOT_ERASE`) — so they return real native JSON instead of `501`. Prompt-cache stats readout is covered by M2's `getPromptCacheHits` / `SlotMetrics.getCacheHitRate`.
...
## 4. Version-bump dependency — corrected

The original assumption was that the `GIT_TAG` bump alone would surface the gated features.
That is **only true for per-device utilization**: the Java `ServerMetrics` getters already parse a
`devices` array, so once the pinned llama.cpp emits it they light up with no Java change. (The
pinned tag is already **b9964**, newer than the b9941 assumed when this plan was written; the
`devices` array is still not emitted by `server_task_result_metrics::to_json`, so that field
remains deferred until a further upstream change.)

For the remaining §2.1 routes the blocker is **not the tag — it is missing JNI bindings in
`src/main/cpp/jllama.cpp`**:

- **`/v1/audio/transcriptions`** — `jllama.cpp` has TTS (OuteTTS) but **no Whisper
  speech-to-text** path at all. Implementing it needs a multimodal audio-input + Whisper decoding
  pipeline in the native layer (a substantial feature, not a wiring change).
- **`/v1/chat/completions/control`** — `jllama.cpp` drives tasks through its own scheduler
  (`requestCompletion` / `receiveCompletionJson` / `cancelCompletion`), not the upstream
  `SERVER_TASK_TYPE_CONTROL` channel. Exposing pause/resume/set-params needs native control
  plumbing against the server context.
- **`POST/DELETE /slots`** — **done.** The native `handleSlotAction` (`SERVER_TASK_TYPE_SLOT_SAVE`
  / `SLOT_ERASE`) already exists and is now wired through `LlamaModel.saveSlot` / `eraseSlot`.

Bottom line: only per-device metrics and slot admin were ever realistically gated; slot admin is
complete, per-device awaits an upstream metrics change, and audio/control require genuine native
feature work before they can leave the `501 Not Implemented` stub.
