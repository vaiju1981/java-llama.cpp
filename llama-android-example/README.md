...
-->
# llama.cpp Android example app

A minimal, **KISS local-AI** Android chat demo built on
...
Nothing else. It is deliberately one screen. Use an **instruct/chat** GGUF (e.g. a small
Qwen/Llama/Gemma instruct model) for coherent replies.
...
`MANAGE_EXTERNAL_STORAGE`, no `READ_EXTERNAL_STORAGE`. (Large models take a moment to
copy; the UI shows a loading state.)
...
```bash
# The example resolves the AAR + Kotlin façade from mavenLocal, so publish them first
...
#   (CI does this automatically — see .github/workflows/publish.yml build-android-example).
...
gradle -p llama-android-example assembleDebug -PjllamaVersion="$VERSION"

# Release App Bundle for Google Play (.aab):
gradle -p llama-android-example bundleRelease -PjllamaVersion="$VERSION"
```

`-PjllamaVersion` selects which `llama-android`/`llama-kotlin` version to consume; omit it
to use the default pinned in `app/build.gradle.kts`. Opening the `llama-android-example/`
...
The `build-android-example` job in `.github/workflows/publish.yml`:
...
2. Builds a **release `.aab`** — signed with the upload key when the CI secrets are set
   ...
   password secrets), otherwise debug-signed — and uploads it as the `llama-android-example-aab`
   artifact.
3. Runs the app's **instrumented UI test on a KVM-accelerated x86_64 emulator** (API 30, the
   same runner as `test-android-emulator`): it adb-pushes the tiny cached draft model, then
   `connectedDebugAndroidTest` launches the app, **types a prompt, taps Send, and asserts a
   non-empty assistant reply streamed back** — exercising the Compose UI, the `ChatViewModel`,
   the `generateChatFlow` façade, and real native inference from `jni/x86_64/libjllama.so`
   together.
...
└── app/
    ...
    └── src/
        ...
        ├── main/kotlin/net/ladenthin/llama/example/
        │   ├── MainActivity.kt      # Compose UI + SAF model picker
        │   └── ChatViewModel.kt     # model load + streaming chat (the only logic)
        └── androidTest/kotlin/net/ladenthin/llama/example/
